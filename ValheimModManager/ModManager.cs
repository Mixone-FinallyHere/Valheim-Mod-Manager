...
{
    public partial class ValheimModManager
    {
        ...
        /// </summary>
        public static Version version { get; private set; } = typeof(ValheimModManager).Assembly.GetName().Version;

        private static ModuleDefMD thisModuleDef = ModuleDefMD.Load(typeof(ValheimModManager).Module);
        ...
        public partial class ModEntry
        {
                ...
                if (File.Exists(assemblyPath))
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            //if (modDef.TryGetTypeReference("ValheimModManagerNet.ValheimModManager", out var typeRef))
                            //{
                            //    var managerAsmRef = new AssemblyNameReference("ValheimModManager", version);
                            ...
                            if (!cacheExists)
                            {
                                ...
                                {
                                    if (item.FullName == "ValheimModManagerNet.ValheimModManager")
                                    {
                                ...
                                {
                                    if (item.Name == "modsPath" && item.Class.FullName == "ValheimModManagerNet.ValheimModManager")
                                    {
            ...
            MethodInfo FindMethod(string namespaceClassnameMethodname, Type[] types, bool showLog = true)
            {
                ...
                if (!mCache.TryGetValue(key, out var methodInfo))
                {
                    ...
                    {
                        if (showLog)
                            ValheimModManager.Logger.Error($"Can't find method '{namespaceClassnameMethodname}'. Mod '{Info.Id}' is not loaded.");
                    }
        ...
        public static bool Initialize()
        {
            ...
            if (!Assembly.GetExecutingAssembly().Location.Contains($"Managed{Path.DirectorySeparatorChar}ValheimModManager"))
            {
                Logger.Error(@"The ValheimModManager folder must be located only in \Game\*Data\Managed\ directory. This folder is created automatically after installation via ValheimModManager.exe.");
            }
        ...
        private static Assembly CurrentDomain_AssemblyResolve(object sender, ResolveEventArgs args)
        {
            ...
            if (filename != null)
            {
                string filepath = Path.Combine(Path.GetDirectoryName(typeof(ValheimModManager).Assembly.Location), filename);
                if (File.Exists(filepath))
                {
                    try
                    {
        ...
        private static void _Start()
        {
            ...
            if (Directory.Exists(modsPath))
            {
                ...
                var assemblies = AppDomain.CurrentDomain.GetAssemblies().Where(x => x.ManifestModule.Name == "ValheimModManager.dll");

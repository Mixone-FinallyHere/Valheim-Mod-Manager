...
namespace ValheimModManagerNet.Installer
{
    ...
    public class ModInfo : ValheimModManager.ModInfo
    {
        [JsonIgnore]
    ...
    {
        public const string filename = "ValheimModManagerConfig.xml";
            ...
            if (File.Exists(filename))
            {
                try
                {
                    using (var stream = File.OpenRead(filename))
                    {
                        ...
                        foreach(var file in new DirectoryInfo(Application.StartupPath).GetFiles("ValheimModManagerConfig*.xml", SearchOption.TopDirectoryOnly))
                        {

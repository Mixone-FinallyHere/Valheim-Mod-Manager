...
{
    [Serializable]
    public partial class ValheimModManagerWindow : Form
    {
        const string REG_PATH = @"HKEY_CURRENT_USER\Software\valheimModManager";

        private static readonly Version VER_0_13 = new Version(0, 13);
        private static readonly Version VER_0_22 = new Version(0, 22);

        public ValheimModManagerWindow()
        {
            InitializeComponent();
            Load += ValheimModManagerWindow_Load;
        }

        private void ValheimModManagerWindow_Load(object sender, EventArgs e)
        {
            Init();
            InitPageMods();
        }
        ...
        static readonly Dictionary<string, LibIncParam> libraryFiles = new Dictionary<string, LibIncParam>
        {
            ...
            { nameof(ValheimModManager) + ".dll", LibIncParam.Normal },
            { nameof(ValheimModManager) + ".xml", LibIncParam.Normal },
        };

        static List<string> libraryPaths;

        public static ValheimModManagerWindow instance = null;
        ...
        private void Init()
        {
                    ...
                    if (registry != null)
                    {
                        ...
                        if (getValue != null)
                        {
                            ...
                            if (string.IsNullOrEmpty(exePath) || !File.Exists(exePath))
                            {
                                ...
                                if (setValue != null)
                                {
                                    setValue.Invoke(null, new object[] { REG_PATH, "ExePath", Path.Combine(Application.StartupPath, "ValheimModManager.exe") });
            ...
            version = typeof(ValheimModManager).Assembly.GetName().Version;
        ...
        private void ValheimModLoaderWindow_WindowClosing(object sender, FormClosingEventArgs e)
        {
            if (config == null)
                return;
        ...
        private void RefreshForm()
        {
            ...
            managerPath = Path.Combine(managedPath, nameof(ValheimModManager));
            entryAssemblyPath = Path.Combine(managedPath, assemblyName);
            injectedEntryAssemblyPath = entryAssemblyPath;
            managerAssemblyPath = Path.Combine(managerPath, typeof(ValheimModManager).Module.Name);
            ...
            var managerType = typeof(ValheimModManager);
            var starterType = typeof(Injection.ValheimModManagerStarter);
            ...
            if (managerInstalled != null && (hasInjectedAssembly || selectedGameParams.InstallType == InstallType.DoorstopProxy))
            {
                ...
                {
                    var versionString = managerInstalled.Fields.First(x => x.Name == nameof(ValheimModManager.version)).Constant.Value.ToString();
        ...
        //                        {
        //                            cryptoClasses.Add(new XElement("cryptoClass", new XAttribute("ummRngWrapper", "ValheimModManagerNet.RngWrapper, ValheimModManager, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null")));
        //                        }
        //                    }
                ...
                case Actions.Install:
                    {
                        try
                        {
                            ...
                            var instr = OpCodes.Call.ToInstruction(starter.Methods.First(x => x.Name == nameof(Injection.ValheimModManagerStarter.Start)));
                ...
                case Actions.Remove:
                    {
                        try
                        {
                            ...
                            if (v0_12_Installed != null || newWayInstalled != null)
                            {
                                ...
                                {
                                    instr = OpCodes.Call.ToInstruction(newWayInstalled.Methods.First(x => x.Name == nameof(Injection.ValheimModManagerStarter.Start)));
                                }
                                else if (v0_12_Installed != null)
                                {
                                    instr = OpCodes.Call.ToInstruction(v0_12_Installed.Methods.First(x => x.Name == nameof(ValheimModManager.Start)));
                                }
        ...
        //{
        //    return doc.Root.Element("mscorlib").Attribute(nameof(ValheimModManager)) != null;
        //}

        //private static void MakeDirty(XDocument doc)
        //{
        //    doc.Root.Element("mscorlib").SetAttributeValue(nameof(ValheimModManager), ValheimModManager.version);
        //}

        private static bool IsDirty(ModuleDefMD assembly)
        {
            return assembly.Types.FirstOrDefault(x => x.FullName == typeof(Marks.IsDirty).FullName || x.Name == typeof(ValheimModManager).Name) != null;
        }

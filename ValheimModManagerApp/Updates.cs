...
namespace ValheimModManagerNet.Installer
{
    ...
    {
        readonly Dictionary<GameInfo, HashSet<ValheimModManager.Repository.Release>> repositories = new Dictionary<GameInfo, HashSet<ValheimModManager.Repository.Release>>();
        ...
        private void CheckModUpdates()
        {
            ...
            if (!repositories.ContainsKey(selectedGame))
                repositories.Add(selectedGame, new HashSet<ValheimModManager.Repository.Release>());
        ...
        private void ModUpdates_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e, GameInfo game, string url)
        {
            ...
            if (!e.Cancelled && !string.IsNullOrEmpty(e.Result) && repositories.ContainsKey(game))
            {
                try
                {
                    var repository = JsonConvert.DeserializeObject<ValheimModManager.Repository>(e.Result);
        ...
        private void LastVersion_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            ...
            if (!e.Cancelled && !string.IsNullOrEmpty(e.Result))
            {
                try
                {
                    ...
                    var release = repository.Releases.FirstOrDefault(x => x.Id == nameof(ValheimModManager));
